...
FT_W32_CreateFile() / FT_OpenEx()//Opens the handle to the specified FTDI chip connection
FT_W32_WriteFile() / FT_Write()//Sends data over virtual COM port.
FT_W32_ReadFile() / FT_Read()//Reads data from virtual COM port
FT_W32_CloseHandle() / FT_Close()//Closes connection handle.

There are functions that allow to set up the port:
FT_SetBaudRate()//Sets the baud rate for the connection
FT_SetDataCharacteristics()//Sets the number of bits in the byte, parity, etc
FT_ClrDtr()//Clears DTR signal on the virtual COM port
FT_SetDtr()//Sets DTR signal on the virtual COM port 
FT_ClrRts()//Clears RTS signal on the virtual COM port 
FT_SetRts()//Sets RTS signal on the virtual COM port 
 * 
 * ************************************************************/
