		...
		if kb.kbhit():
			try:
				...
				if (k_in == '8'): # A': # if (k_in == 1):
					...
					setFocus = setFocus - stepSize
					focus = focus - stepSize#0.001
					writeup_location(setOffset, focus[0], stepSize, setWidth) #print('offset = %5.3f, focus = %5.3f' % (scan[0], focus[0]))
				...
				# [keys 1 2 3 4] change stepSize -----------------------
					...
					writeup_location(setOffset, focus[0], stepSize, setWidth) #sys.stdout.write(' \roffset = %5.3f, focus = %5.3f   |step size = %5.3f     |' % (scan[0], focus[0], stepSize))
					#sys.stdout.flush()
				#elif (k_in =='x'): #2': #'Q':
					#print('[F2] PRESSED')
					#stepSize = 0.01
				...
				#<< width alpha keys Inputs
					...
					"""
					scan = np.concatenate((scan,scan[::-1]),0)
					writeup_location(setOffset, focus[0], stepSize, setWidth) #setWidth) #sys.stdout.write(' \roffset = %5.3f, focus = %5.3f   |step size = %5.3f     |' % (scan[0], focus[0], stepSize))
					...
					setWidth = 0.
					scan = np.linspace(-1*setWidth/2, setWidth/2, samples/2.)
					scan = np.concatenate((scan,scan[::-1]),0)
					writeup_location(setOffset, focus[0], stepSize, setWidth)
