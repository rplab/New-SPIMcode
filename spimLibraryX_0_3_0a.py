...
17 June 2019 writeData = np.concatenate((focus, scan),0 changed back to writeData = np.concatenate((focus, scan),1
...
- Added [t] and [y] to reset focus/offset and beam width to zero
- Beam width can no longer go below zero

8 August 2019 version 0_3_0a changes:
- Added [p] and [l] to save current galvo settings and then load them

Thoughts:
- Get rid of excess code? Only seems like we're using two Functions
- Make variable naming consistent and concise
"""
...
def no_scan_writeup_location(setOffset, setFocus, stepSize):
	# over writes last line in terminal instead of scrolling down
	sys.stdout.write(' \r  |  offset = %5.3f :: focus = %5.3f   |  step size = %5.3f   |' %(setOffset, setFocus, stepSize)) #scan, focus, stepSize))
	sys.stdout.flush()
...
def functionTest():
	...
	sys.stdout.flush()
	time.sleep(0.1)
'''Common Two Functions ========================================================================================================'''
def ContinuousScan2(setOffset, setFocus, stepSize, duration=10. , setWidth=0. , focusChannel="Dev1/ao2", scanChannel="Dev1/ao3"):
	#<< comments
	...
	***To Do:
	...
	Inputs:
	    ...
	    focusChannel - string, Hardware device and channel that controls the focus galvo, e.g. "Dev1/ao2"
	    scanChannel - string, Hardware device and channel that controls the scan galvo, e.g. "Dev1/ao3"
	"""
	#>> end comments
	...
	scan = np.linspace(-1*setWidth/2, setWidth/2, samples/2.) + offset
	...
	if (focusChannel == "Dev1/ao2"):
		writeData = np.concatenate((focus, scan),0)
	else:
		writeData = np.concatenate((scan, focus),0)
		...
		if kb.kbhit():
			try:
				...
				if (k_in == '8'): # A': # if (k_in == 1):
					...
					setFocus = setFocus - stepSize
					focus = setFocus * np.ones(samples)#0.001
					writeup_location(setOffset, setFocus, stepSize, setWidth) #print('offset = %5.3f, focus = %5.3f' % (scan[0], focus[0]))
				elif (k_in == '6'): #C'): # elif (k_in == 0):
					setOffset = setOffset + stepSize
					...
					focus = setFocus * np.ones(samples)
				...
				# [keys 1 2 3 4] change stepSize -----------------------
					...
					writeup_location(setOffset, setFocus, stepSize, setWidth) #sys.stdout.write(' \roffset = %5.3f, focus = %5.3f   |step size = %5.3f     |' % (scan[0], focus[0], stepSize))
					#sys.stdout.flush()
				#elif (k_in =='x'): #2': #'Q':
					#print('[F2] PRESSED')
					#stepSize = 0.01
					#print('Step Size now = ', stepSize)
					#writeup_location(scan[0], focus[0], stepSize, extent) # sys.stdout.write(' \roffset = %5.3f, focus = %5.3f   |step size = %5.3f     |' % (scan[0], focus[0], stepSize))
					#sys.stdout.flush()
				#elif (k_in =='c'): #3': # 'R':
					#print('[F3] PRESSED')
					#stepSize = 0.1
				...
				#<< width alpha keys Inputs
					...
					setWidth = setWidth + stepSize #setWidth
					#print('[F4] PRESSED')
					scan = np.linspace(-1*setWidth/2, setWidth/2, samples/2.) + setOffset
					"""Trace backward for less acceleration on the galvo (pyramid instead of sawtooth):
					"""
					scan = np.concatenate((scan,scan[::-1]),0)
					writeup_location(setOffset, setFocus, stepSize, setWidth) #setWidth) #sys.stdout.write(' \roffset = %5.3f, focus = %5.3f   |step size = %5.3f     |' % (scan[0], focus[0], stepSize))
					...
					if setWidth - stepSize >= 0:
						setWidth = setWidth - stepSize #
					elif setWidth - stepSize < 0:
						setWidth = 0.
					#print('[F4] PRESSED')
					scan = np.linspace(-1*setWidth/2, setWidth/2, samples/2.) + setOffset
					"""Trace backward for less acceleration on the galvo (pyramid instead of sawtooth):
...
def NoScanFocus2getch(setOffset, setFocus, stepSize, setWidth, DebugMode, focusChannel="Dev1/ao2", scanChannel="Dev1/ao3"):
	#<< comments
	"""13th May 2019, mhd:
	...
	***To Do:
	...
	print('    [q] to exit back to to menu.')
	offset = setOffset
	focus = setFocus
	storedOffset = 0
	storedFocus = 0
	no_scan_writeup_location(setOffset, setFocus, stepSize)
	if DebugMode == 'Off':
		analog_output = pydaq.Task()
		analog_output.CreateAOVoltageChan(focusChannel, "", -10.0, 10.0, pydaq.DAQmx_Val_Volts, None)
		analog_output.CreateAOVoltageChan(scanChannel, "", -10.0, 10.0, pydaq.DAQmx_Val_Volts, None)
...
#				print('~line 626, try kbhit:')
				...
				if (k_in == '8'): # A': # if (k_in == 1):
					...
					setFocus = setFocus - stepSize
					focus = focus - stepSize #0.001
					no_scan_writeup_location(setOffset, setFocus, stepSize) #print('offset = %5.3f, focus = %5.3f' % (scan[0], focus[0]))
				elif (k_in == '6'): #C'): # elif (k_in == 0):
					setOffset = setOffset + stepSize
					scan = scan + stepSize #0.001
					no_scan_writeup_location(setOffset, setFocus, stepSize) #print('offset = %5.3f, focus = %5.3f' % (scan[0], focus[0]))
					...
					setFocus = storedFocus
					focus = setFocus * np.ones(samples)
					scan = setOffset * np.ones(samples)
					no_scan_writeup_location(setOffset, setFocus, stepSize)
				...
				# [keys 1 2 3 4] change stepSize -----------------------
					...
					if stepSize == 0.001:
						...
						stepSize = 0.001
					#print('Step Size now = ', stepSize)
					no_scan_writeup_location(setOffset, setFocus, stepSize) #sys.stdout.write(' \roffset = %5.3f, focus = %5.3f   |step size = %5.3f     |' % (scan[0], focus[0], stepSize))
					#sys.stdout.flush()
				#elif (k_in =='x'): #2': #'Q':
					#print('[F2] PRESSED')
					#stepSize = 0.01
...
#<< OLDER Not used much if at all functions ======================================================================================
def TestScanMultiFocus(focus, offset, exposure, slices, colors, timepoints, focusChannel="Dev1/ao2", scanChannel = "Dev1/ao3"):
...
def Scan(focus, offset, images, exposure, readout=10., focusChannel="Dev1/ao2", scanChannel="Dev1/ao3"):
...
def Scan2(setOffset, setFocus, stepSize, setWidth, images, exposure, readout=10., focusChannel="Dev1/ao2", scanChannel="Dev1/ao3"):
	#<< comments
	"""Function to scan galvo mirror across field of view, centered on offset voltage and with the focus set by the perpendicular galvo. This function is specific to a National Instruments DAQ card, used to send signals to the mirror galvonometers. A trigger (e.g. from the camera) is currently necessary on PFI0, if undesirable, change CfgDigEdgeStartTrig.
...
def ContinuousScan(focus, offset, duration=10. , extent=0.500 , focusChannel="Dev1/ao2", scanChannel="Dev1/ao3"):
	"""Function to continuously scan galvo mirror across field of view, centered on offset voltage and with the focus set by the perpendicular galvo. This function is specific to a National Instruments DAQ card, used to send signals to the mirror galvonometers. This function is useful when the camera is in free-runnung mode for, e.g. sample location, focus adjustment, etc.
...
def NoScanFocus(focus, offset, focusChannel="Dev1/ao2", scanChannel="Dev1/ao3"):
	"""Function to hold scan galvo mirror in middle of field of view (at offset voltage) and with the initial focus set by the perpendicular galvo. This function is specific to a National Instruments DAQ card, used to send signals to the mirror galvonometers. This function is used when the camera is in free-runnung mode for focus adjustment, taking keyboard input to adjust the focus voltage.
	...
	***To Do:
	...
	if (focusChannel == "Dev1/ao2"):
		#writeData = np.concatenate((focus, scan),0)
		writeData = np.concatenate((focus, scan),0) # IndexError: axis 1 out of bounds [0, 1), 6 june 2019, mhd
	else:
		writeData = np.concatenate((scan, focus),0)
...
def NoScanFocus2getarrow(setOffset, setFocus, stepSize, setWidth, focusChannel="Dev1/ao2", scanChannel="Dev1/ao3"):
	"""Function to hold scan galvo mirror in middle of field of view (at offset voltage) and with the initial focus set by the perpendicular galvo. This function is specific to a National Instruments DAQ card, used to send signals to the mirror galvonometers. This function is used when the camera is in free-runnung mode for focus adjustment, taking keyboard input to adjust the focus voltage.
        11 MAy 2019, mhd copied NoScanFocus() to NoScanFocus2getch() repalce getarrow() with getch()
        getarrow() can not get other keys than arrows.
        added f1,f2,f3,f4 keys to change stepSize
...
def NoScanFocus2getarrowDEBUG(setOffset, setFocus, stepSize, setWidth, focusChannel="Dev1/ao2", scanChannel="Dev1/ao3"):
	...
	***To Do:
	...
	Inputs:
	    ...
	    scanChannel - string, Hardware device and channel that controls the scan galvo, e.g. "Dev1/ao3"
	"""
	print('arrow keys active to change focus and offset, [q] to exit back to to menu.')
	#analog_output = pydaq.Task()
	#analog_output.CreateAOVoltageChan(focusChannel, "", -10.0, 10.0, pydaq.DAQmx_Val_Volts, None)
	#analog_output.CreateAOVoltageChan(scanChannel, "", -10.0, 10.0, pydaq.DAQmx_Val_Volts, None)
	offset = setOffset
...
def galvoHiLoScan(focus, offset, stripewidth, duration, focusChannel="Dev1/ao2", scanChannel="Dev1/ao3"):
...
def aotfModScan(focus, offset, duration, aomV, aomStripeWidth, phase=0, focusChannel="Dev1/ao2", scanChannel="Dev1/ao3", aomChannel="Dev1/ao2"):
...
#	aom = np.concatenate((aom, aom[::-1]),0)
	...
	if (focusChannel == "Dev1/ao2"):
		writeData = np.concatenate((focus, scan, aom, blank),0)
	else:
		writeData = np.concatenate((scan, focus, aom, blank),0)
...
#	return
#>> END OF OLDER Functions section ===========================================
